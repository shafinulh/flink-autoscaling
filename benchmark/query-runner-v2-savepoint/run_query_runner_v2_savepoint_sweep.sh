...
submit_job() {
  ...
  log "Submitting ${JOB_NAME} with savepoint ${savepoint} (tm.process=${tm_size})..." | tee -a "$log_file"
  if [[ $SETSID_AVAILABLE == true ]]; then
    nohup setsid "${RUN_QUERY_CMD}" "oa" "$JOB_NAME" "$savepoint" >>"$log_file" 2>&1 &
  else
    nohup "${RUN_QUERY_CMD}" "oa" "$JOB_NAME" "$savepoint" >>"$log_file" 2>&1 &
  fi
  current_job_pid=$!
  if [[ $SETSID_AVAILABLE == true ]]; then
...
write_metadata() {
  ...
  local tm_size=$2
  local savepoint=$3
  local savepoint_tag=$4
  local experiment_name=$5
  local experiment_label=$6
  local meta_file="${run_dir}/metadata.txt"
  {
    ...
    echo "savepoint_tag=${savepoint_tag}"
...
for tm_idx in "${!TM_MEMORY_SIZES[@]}"; do
  ...
  write_metadata "$run_dir" "$tm_size" "$SAVEPOINT_INPUT" "$savepoint_tag" "$EXPERIMENT_NAME" "$EXPERIMENT_LABEL"
  submit_job "$run_dir" "$tm_size" "$SAVEPOINT_INPUT"
  job_start_ts=$(date +%s)
