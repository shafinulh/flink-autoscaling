...
DEFAULT_PROMQL_THROUGHPUT = (
    "sum by (job_name, operator_name) ("
    "flink_taskmanager_job_task_operator_numRecordsOutPerSecond"
    "{cluster=\"nexmark-flink\", job_name=~\"$job\", operator_name=~\".*Source.*\"}"
    ")"
)
DEFAULT_PROMQL_HIT_RATIO = (
    "100 * rate("
    "flink_taskmanager_job_task_operator_rocksdb_block_cache_hit"
    "{cluster=\"nexmark-flink\", job_name=~\"$job\"}[1m])"
    " / ("
    "rate(flink_taskmanager_job_task_operator_rocksdb_block_cache_hit"
    "{cluster=\"nexmark-flink\", job_name=~\"$job\"}[1m])"
    " + rate(flink_taskmanager_job_task_operator_rocksdb_block_cache_miss"
    "{cluster=\"nexmark-flink\", job_name=~\"$job\"}[1m])"
    ")"
)
...
def prepare_promql(query, job_regex):
    if not query:
        return ""
    return query.replace("$job", job_regex)


def _prom_query_range(prom_url, query, start_micros, end_micros, step_seconds):
    start_sec = start_micros / 1_000_000
    end_sec = end_micros / 1_000_000
    params = {
        "query": query,
...
def fetch_prom_series(prom_url, query, start_micros, end_micros, step_seconds):
    if not prom_url or not query:
        return {"label": "", "points": []}
    data = _prom_query_range(prom_url, query, start_micros, end_micros, step_seconds)
    result = data.get("result", [])
    if not result:
        return {"label": query, "points": []}
    series = result[0]
    points = []
    for stamp, value in series.get("values", []):
        try:
            time_micros = int(float(stamp) * 1_000_000)
            points.append({"time_micros": time_micros, "value": float(value)})
        except (ValueError, TypeError):
            continue
    return {"label": query, "points": points}
...
def get_log_time_range(lsm_frames, stats_dumps):
    times = [dump["time_micros"] for dump in stats_dumps if dump.get("time_micros")]
    if not times:
        times = [frame["time_micros"] for frame in lsm_frames if frame.get("time_micros")]
    if not times:
        return None, None
...
def build_series_for_experiment(
    log_path,
    lsm_frames,
    stats_dumps,
    prom_url,
    ...
    default_offset = int(series_offset_hours * 3600 * 1_000_000)
    log_start, log_end = get_log_time_range(lsm_frames, stats_dumps)

    series = {
        "throughput": {"label": "", "points": []},
        "block_cache_hit_ratio": {"label": "", "points": []},
    }
    prom_used = False

    if prom_url and log_start is not None and log_end is not None:
        series_start = log_start + default_offset
        series_end = log_end + default_offset
        try:
            series["throughput"] = fetch_prom_series(
                prom_url, promql_throughput, series_start, series_end, prom_step_seconds
            )
            series["block_cache_hit_ratio"] = fetch_prom_series(
                prom_url, promql_hit_ratio, series_start, series_end, prom_step_seconds
            )
            prom_used = bool(series["throughput"]["points"] or series["block_cache_hit_ratio"]["points"])
        except Exception:
            series = {
                "throughput": {"label": "", "points": []},
                "block_cache_hit_ratio": {"label": "", "points": []},
    ...
    if not series["throughput"]["points"] and not series["block_cache_hit_ratio"]["points"]:
        throughput_csv, hit_ratio_csv = find_csv_files(Path(log_path).parent)
        series = {
            "throughput": parse_csv_series(throughput_csv) if throughput_csv else {"label": "", "points": []},
            "block_cache_hit_ratio": parse_csv_series(hit_ratio_csv)
            if hit_ratio_csv
            else {"label": "", "points": []},
        }

    time_offset_micros = default_offset
    if not prom_used and log_start is not None:
        series_times = []
        for points in (series["throughput"]["points"], series["block_cache_hit_ratio"]["points"]):
            if points:
                series_times.append(points[0]["time_micros"])
        if series_times:
            time_offset_micros = min(series_times) - log_start

    return series, time_offset_micros, log_start, log_end
...
def _slugify(value):
    ...
    return safe.strip("_") or "log"


def _extract_operator_name(log_path):
    name = Path(log_path).name
    marker = "_op_"
    if marker not in name:
        return "unknown-operator"
    after = name.split(marker, 1)[1]
    return after.split("__", 1)[0] or "unknown-operator"
    ...
    for child in sorted(root.iterdir()):
        ...
        for candidate in candidates:
            logs.append(
                {
                    "path": candidate,
                    "operator": _extract_operator_name(candidate),
                }
            )
        experiments.append((child.name, logs))
    return experiments
...
def run_server(static_dir, host, port, prom_url):
    class Handler(SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if parsed.path == "/api/query_range":
                ...
                start = _parse_time_value(params.get("start", [None])[0])
                end = _parse_time_value(params.get("end", [None])[0])
                step = params.get("step", [None])[0]
                try:
                    step_seconds = int(step) if step else 10
                except ValueError:
                    step_seconds = 10

                if not prom_url:
                    self.send_error(400, "Prometheus URL not configured")
                    return
                if not query or start is None or end is None:
                    self.send_error(400, "Missing query/start/end")
                    return

                try:
                    series = fetch_prom_series(prom_url, query, start, end, step_seconds)
                except Exception as exc:
                    self.send_error(500, f"Prometheus query failed: {exc}")
                    return

                payload = json.dumps(series).encode("utf-8")
...
def main():
    ...
    parser.add_argument(
        "--promql-throughput",
        help="PromQL query for source throughput",
    )
    parser.add_argument(
        "--promql-hit-ratio",
        help="PromQL query for block cache hit ratio",
    )
    parser.add_argument(
        "--job-regex",
        default=".*",
        help="Regex for job_name in PromQL (used to replace $job)",
    )
    parser.add_argument(
        "--promql-step",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=-5.0,
        help="Shift RocksDB times to match monitoring data (hours)",
    )

    args = parser.parse_args()
    base_dir = Path(__file__).resolve().parent
    static_dir = base_dir / "static"
    promql_throughput = prepare_promql(
        args.promql_throughput or DEFAULT_PROMQL_THROUGHPUT, args.job_regex
    )
    promql_hit_ratio = prepare_promql(
        args.promql_hit_ratio or DEFAULT_PROMQL_HIT_RATIO, args.job_regex
    )
    ...
    if args.root:
        ...
        experiments = []
        for name, log_entries in find_experiment_logs(args.root):
            log_items = []
            for entry in log_entries:
                log_path = entry["path"]
                operator = entry["operator"]
                data = parse_experiment(log_path, args.left_name, args.right_name, args.max_levels)
                series, time_offset_micros, log_start, log_end = build_series_for_experiment(
                    log_path,
                    data["lsm"]["frames"],
                    data["stats"]["dumps"],
                    args.prometheus_url,
                ...
                data["experiment"] = {
                    ...
                    "stats_time_start_micros": log_start,
                    "stats_time_end_micros": log_end,
                }
                filename = f"{_slugify(name)}__{_slugify(operator)}.json"
            ...
            experiments.append({"name": name, "logs": log_items})
    ...
    run_server(static_dir, args.host, args.port, args.prometheus_url)
